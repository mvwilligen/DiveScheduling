...
def appointmentsdate2(text, cFrom):
    ...
    if form.validate_on_submit():
        ...
        for a in filtered:
            ...
            cAssistantValue = request.form.get(eval(cAssistantField))
...
def appointmentsevents(cName, cDate, cFrom):
    ...
    if form.validate_on_submit():
        ...
        cNewstudent = request.form.get("newstudent")
        ...
        for a in appointments2:
            ...
            print(cAssistant)
            cAssistantValue = request.form.get(eval(cAssistant))
        ...
        if len(cNewstudent) > 0:
            ...
            cFormTime = request.form.get('time00')
            cFormDate = request.form.get('date00')
            cAssistantForm = request.form.get('assistant00')

            # 20240916?
            # cInstructor = chr(34) + "instructor" + str(a.Id) + chr(34)
            # print(cInstructor)
            cInstructorValue = request.form.get('instructor00')
            nFound = 0
            for i in instructors2:
                print('-- i:',i[0], i[1], i[2])
                if cInstructorValue == i[2]:
...
def appointmentsnotes(text, cFrom):
    ...
    for a in appointments:
        ...
        cText = cText + "<br>" + cCRLF
        cText = cText + "student: " + a[2] + " " + a[3] + "<br>" + cCRLF
        cText = cText + "----------------------------------------------------------------------------------------------------------------------<br>" + cCRLF

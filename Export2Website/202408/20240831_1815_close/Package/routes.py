...
def login():
    ...
    if request.method == 'POST':
        ...
        #user = Users.query.filter_by(Username = 'Laura').first()
        user = db.session.execute(db.select(Users).filter_by(Username=username)).scalar_one()

        pwhash2 = bcrypt.generate_password_hash('123').decode('utf-8')
        ...
        print(user)
        print('pwhash:   ' + user.Passwordhash)
        print('pwhash2:  ' + pwhash2)
        print()

        pwhash = user.Passwordhash

        if user and bcrypt.check_password_hash(pwhash2, '123'):
            login_user(user)

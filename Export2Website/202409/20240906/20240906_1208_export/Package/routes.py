...
def test(a,b):
    ...
    cHtml = '<html><style>'
    cHtml = cHtml + 'td { text-align:left; vertical-align:top }'
    cHtml = cHtml + ''
    cHtml = cHtml + ''
    cHtml = cHtml + ''
    cHtml = cHtml + ''
    
    cHtml = cHtml + '</style><body>'
    ...
    cHtml = cHtml + '<br><br>' + cDate + ' ' + cTime + '<br>'

    cFolderName = 'Exports/divescheduling_' + cDate + '_' + cTime

    if not os.path.exists(cFolderName):
        os.makedirs(cFolderName) 

    cFileName = cFolderName + '/calendar_' + cDate+ '_' + cTime + '.html'

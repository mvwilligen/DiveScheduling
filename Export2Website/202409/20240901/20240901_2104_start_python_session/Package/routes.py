...
from flask_login import LoginManager, current_user # added current_user later...
...
def homepage():
    ...
    #                                        1                2                  3                4               5                     6                  7                  8 
    appointments = db.session.execute( db.select(Appointments.Id, Appointments.User, Users.Firstname, Users.Lastname, Products.Productname, Appointments.Part, Appointments.Date, Instructors.Name). \
...
def appointmentsassign(id):
    ...
    appointment = db.session.execute(db.select(Appointments).filter_by(Id=id)).scalar_one()
    #print ('appointment.Part:  ' + appointment.Part + ' ' + appointment.Date.strftime('%y-%m-%d'))
    #print ('appointment.Staff: ' + str(appointment.Staff))
    #print ('instructor.Id:     ' + str(instructor.Id))
    #print ('instructor.User:   ' + str(instructor.User))
...
def login():
    ...
    if request.method == 'POST':
        ...
        # user = user._asdict() ## <class 'dict'>
        print(user)
        print('len(user2):        ', len(user2))
        print('type(user):        ', type(user))
        print('type(user2):       ', type(user2))
        print('user.Username:     ', user2 [0])
        print('len(user2):        ', len(user2[0]))
#        print('user.Passwordhash: ' + user2 [2])
        print('-----------------------------------------')

        pwhash = user2.Passwordhash

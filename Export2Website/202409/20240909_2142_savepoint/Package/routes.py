...
def usersmail(id):
    ...
    if 'admin' in lRBAC[1]:
        ...
        cFilename = "C:/Data/Python/DiveScheduling/Exports/Mail/calendar_st_" + cFullname2 + ".html"
        if os.path.isfile(cFilename):
            cMailfile = cFilename

        cFilename = "C:/Data/Python/DiveScheduling/Exports/Mail/calendar_in_" + cFullname2 + ".html"
        if os.path.isfile(cFilename):
            cMailfile = cFilename
        ...
        if len(cMailfile) > 0:
            ...
            cMessageText = strip_tags(cMessageHtml)
...
def htmlexport():
    ...
    if not os.path.exists('Exports/Mail'):
        os.makedirs('Exports/Mail') 
    ...
    cHtmlCal = '<html>' + cStyle + '<body>[cInstructorsMenu]<br>' + cCRLF
        ...
        for nMonths in range(nMonthFrom, nMonthTo):
            ...
            if True:
                ...
                for nDays in range(nLastDay):
                    ...
                    cAddHtmlHeader = "<tr><td style='width:200px'><b>date</b></td><td style='width:200px'><b>product</b></td><td style='width:200px'><b>part</b></td><td style='width:200px'><b>student</b></td><td style='width:200px'><b>instructor</b></td></tr>" + cCRLF

                    for a in appointments:
 
                        # date html
                        cHtmlDat = cHtmlDat + '<tr><td>' + a [6].strftime("%d-%m-%Y %H:%M") + cWhiteSpace  + '</td><td>' + a [4] + cWhiteSpace + '</td><td>' + a [5] + cWhiteSpace + '</td><td>'  + a [2] + ' ' + a [3] + cWhiteSpace + '</td><td>' + a [8] + cWhiteSpace + '</td>' + cCRLF
                        ...
                        # html student
                        cAddHtmlLine = "<tr><td>" + a [6].strftime("%d-%m-%Y %H:%M") + "</td><td>" + a [4] + "</td><td>" + a [5] + "</td><td>" + a [2] + " " + a [3] + "</td><td>" + a [8] + "</td></tr>" + cCRLF
                        ...
                        # html instructor
                        cAddHtmlIns = "<tr><td>" + a [6].strftime("%d-%m-%Y %H:%M") + "</td><td>" + a [4] + "</td><td>" + a [5] + "</td><td>" + a [2] + " " + a [3] + "</td><td>" + a [8] + "</td></tr>" + cCRLF
                    ...
                    if len(cHtmlDat) > 0:
                        # date html
                        cHtmlDat2 = '<html>' + cStyle + '<body>[cInstructorsMenu]<br><br><table>' + cCRLF

                        cWeekdayName = lDayNames[datetime.date(nYear, nMonth, nDay).weekday()]

                        cHtmlDat2 = cHtmlDat2 + "<tr><td colspan=5 style='text-align: center;'><h3 style='margin-bottom: 0;'>" + str(nDay) + " " + lMonthNames[nMonth] + " " + str(nYear) + " - " + cWeekdayName + "</h3></td></tr>" + cCRLF
                        cHtmlDat2 = cHtmlDat2 + "<tr><td style='width:200px'><b>time</b><td style='width:200px'><b>product</b><td style='width:200px'><b>part</b><td style='width:200px'><b>student</b><td style='width:200px'><b>instructor</b></td><tr>" + cCRLF
    ...
    for i in aStudentHtml:
        ...
        cHtmlIns = "<html>" + cStyle + "<body>[cInstructorsMenu]<br><br><table><tr><td colspan=5 style='text-align: center;'><h3 style='margin-bottom: 0;'>" + i [2] + "</h3></td></tr>" + i [3] + "</table>"
    ...
    cHeader = "<table style='border-style: hidden;'><tr><td style='border:none;'><a href='http://127.0.0.1:5000/'><img src='logo.png' alt='ds'></a></td><td style='border:none;'>" + cCRLF
    cHeader = cHeader + cInstructorsMenu  + cCRLF
    cHeader = cHeader + '<br>' + cCRLF
    cHeader = cHeader + cStudentMenu  + cCRLF
    cHeader = cHeader + '</td></tr></table><br>'

    cFolderNameOSHtml = './Exports/divescheduling_' + cDate4 + '_' + cTime + '/'
    ...
    for h in aHtmlFiles:
        ...
        cFileName = cFolderNameOSMail + h [0]

        # remove tag for instructorsmenu
        cHtml = h [1].replace('[cInstructorsMenu]', '<br>') 
    ...
    import shutil
    src = 'C:/Data/Python/DiveScheduling/Exports/logo.png'
    # dst = 'C:/Data/Python/DiveScheduling/Exports/...'
    # src = src.replace("\\","/")
    # dst = dst.replace("\\","/")
    dst = cFolderNameOSHtml + 'logo.png'
    ...
    if False:
        ...
        ftp.login(user='lauram@maidiving.nl', passwd = '********')

        listfiles = glob.glob("C:/Data/Python/DiveScheduling/Exports/divescheduling_20240908_1817/*.html")

        for l in listfiles:
            filenamefrom = l
            filenameto = 'divescheduling/' + l[67:]

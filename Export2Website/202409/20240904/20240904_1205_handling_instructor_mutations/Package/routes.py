...
def homepage():
    ...
    #        print('####', "{:04d}".format(a[0]), "{:04d}".format(a[1]), a[2].ljust(12), a[3].ljust(12), a[4].ljust(20), a[5])
    #        list.append(a)
    #
    #    #print('#### type(appointments: ', type(appointments))
    #    #print('#### type(list:         ', type(list))
    #    list.sort(key = lambda x: x[4]) # sort on productname
    #    #print('#### type(list:          ', type(list))
    #    print ('-------------------------------------------------------------------------------------')
    #
    #    for a in list:
...
def registerform2(id):
    ...
    if form.validate_on_submit():
        ...
        if lInstructorBefore and not lInstructorAfter:
            ...
            print('')
            #print('#### instructor.first()[0]                   ', instructor.first()[0])
            #instructor = db.session.execute(db.select(Instructors).filter(User = x.Id)).scalar_one()
            #for i in instructor:
            #    instructor2.append(i)
            #for i in instructor2:
            #    print('#### instructor2 [2]', instructor2 [2])
            #print('#### type(instructor):  scalar_one           ', type(instructor))
            #print('#### instructor.first()[0]                   ', instructor.first()[0])
            #instructor = db.session.execute(db.select(Instructors).filter(User = x.Id)).scalars()
            #for i in instructor:
            #    instructor2.append(i)
            #for i in instructor2:
            #    print('#### instructor2 [2]', instructor2 [2])
            #print('#### type(instructor):  scalars              ', type(instructor))
            #instructor = db.session.execute(db.select(Instructors).filter(User = x.Id))
            #instructor2 = [] # necessary because list could be for/endfor once.
            #for i in instructor:
            #    instructor2.append(i)
            #print('#### type(instructor2):                      ', type(instructor2))
            #for i in instructor2:
            #    print('#### instructor2 [0]', i [0])
            #    #print('#### instructor2 [0]', i [1])
            #print('#### instructor2.first()[0]                   ', instructor2.first()[0])
            #print('#### type(instructor):                       ', type(instructor))
            #print('')
            #print('#### x.Id:                                   ', x.Id, ' (User.Id)')
            #print('#### type(instructor):                       ', type(instructor))
            #print('#### isinstance(instructor, NoneType):       ', isinstance(instructor, type(None)))
            #print('#### len(instructor2):                        ', len(instructor2))
            #print('')
            ##for i in instructor2:
            ...
            dbquery = 'db.session.execute(db.select(Appointments.Id, Appointments.User, Appointments.Product, Appointments.Part, Appointments.Date, Appointments.Notes, Appointments.Staff).where(Appointments.Staff == nStaff)).all()'
            print('#### ', datetime.datetime.today(), ' dbquery: ', dbquery)

...
instructor = db.session.execute(db.select(Instructors).filter(Name = 'Laura Maissan'))
...
instructor = db.session.execute(db.select(Instructors).filter_by(Name = 'LauraMaissan')).scalar_one()
...
user = db.session.execute(db.select(Users).filter_by(Username = 'Laura')).scalar_one()



user = db.session.execute(db.select(Users).filter_by(Username = 'Laura'))
user2 = []
for u in user:
    user2.append(u)

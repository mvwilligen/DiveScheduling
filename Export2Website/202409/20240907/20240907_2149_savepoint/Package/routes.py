...
def htmlexport():
    ...
    cStyle = '<style>'
    cStyle = cStyle + 'td { text-align:left; vertical-align:top; '
    ...
    cHtml = '<html>' + cStyle + '<body>[cInstructorsMenu]<br>'
    ...
    for nYear in range(nYearFrom, nYearTo + 1):
        ...
        for nMonths in range(nMonthFrom, nMonthTo):
            ...
            if nMonth >= nMonthFrom and nMonth <= nMonthTo:
                print('processing month: ', nMonth)

                cHtml = cHtml + "<table>"
                cHtml = cHtml + "<tr><td colspan=8 align='center'><h3>" + lMonthNames [nMonth] + "<h3></td></tr>"
                cHtml = cHtml + '<tr><td><b>week' + cWhiteSpace + '</b></td>'

                for n in lDayNames:
                    cHtml = cHtml + '<td><b>' + n + cWhiteSpace + '</b></td>'
                cHtml = cHtml + '</tr>'

                #print('nMonth: ', nMonth, ' - ' , lMonthNames[nMonth])
                #print('monthrange(nYear, nMonth): ', monthrange(nYear, nMonth))

                #for nDay in monthrange(nYear, nMonth):
                #    print (nDay)

                lResult = monthrange(nYear, nMonth)
                #print('lResult: ', lResult [0], lResult [1] )

                nFirstDay = lResult [0]
                nLastDay  = lResult [1]
                print('nFirstDay: ', nFirstDay, ' - ', lDayNames [nFirstDay])
                print('nLastDay: ', nLastDay)

                # weeknumber
                nDay = 1
                cWeekNumber = datetime.date(int(nYear), int(nMonth), int(nDay)).strftime('%V')
                cWeekNumber = str(int(cWeekNumber) - 1)
                cHtml = cHtml + '<tr><td>' + str(cWeekNumber) + cWeekLines + '</td>'
                ...
                for nDays in range(nLastDay):
                    ...
                    # print (cDate + ' ' + lDayNames [ nDayOfWeek ] [0:3] + ' - ', end='')
                    # cHtml = cHtml + '<td>' + cDate + ' ' + lDayNames [ nDayOfWeek ] [0:3] + '<br>'+str(nDay + 1)+'<br>&nbsp;<br>'
                    cHtml = cHtml + '<td>' + str(nDay) + '<br>' + cWhiteSpace + '<br>'
                    ...
                    for a in appointments:
                        # date html
                        cHtml2 = cHtml2 + '<tr><td>' + a [6].strftime('%H.%M') + cWhiteSpace  + '</td><td>' + a [4] + cWhiteSpace + '</td><td>'  + a [5] + cWhiteSpace + '</td><td>'  + a [2] + ' ' + a [3] + cWhiteSpace + '</td><td>' + a [8] + cWhiteSpace + '</td>' + chr(13) + chr(10)
                        ...
                        # html instructor
                        cAddHtml = "<tr><td>" + a [6].strftime("%d-%m-%Y %H:%M") + cWhiteSpace + "</td><td>" + a [4] + cWhiteSpace + "</td><td>" + a [5] + cWhiteSpace + "</td><td>" + a [2] + " " + a [3] + cWhiteSpace + "</td></tr>"

                        if nFound == 0:
                            nInstructor = nInstructor + 1
                            # print('instructor ' + a [8] + ' not found')
                            cAddHtml = "<tr><td><b>date</b></td><td><b>product</b></td><td><b>part</b></td><td><b>student</b></td></tr>" + cAddHtml
                            aInstructor.append([nInstructor, a [9], a [8], cAddHtml])

                            if not a [8] in cInstructorsMenu:
                                cInstructorsMenu = cInstructorsMenu + "- <a href=" + cFolderNameHtml + "calendar_" + a [8].replace(' ', '_') + ".html>" + a [8] + "</a>&nbsp;"

                            nCounter = 0
                            for i in aInstructor:
                                # print ('i: ', nCounter, i [0], i [1], i [2], i [3])
                                nCounter = nCounter + 1

                            cEmptyTR = cEmptyTR + '|'+ str(a[9]) + '|'

                            print ('adding to menu: ', a [8])
                        else:
                            print('instructor ', a [8],' found: ', nFound)
                            # print('before: ', aInstructor [nFound][2], aInstructor [nFound][3])
                            aInstructor [nFound][3] = aInstructor [nFound][3] + cAddHtml
                            # print('after:  ', aInstructor [nFound][2], aInstructor [nFound][3])

                            cEmptyTR = cEmptyTR + '|'+ str(a[9]) + '|'
                        
                        #print('cEmptyTR: ', cEmptyTR)

                        # print('len(aInstructor): ', len(aInstructor))
                        #---------------------------------------

                        if not a [4] in cProducts:
                            cHtml = cHtml + '<a href=' + cFolderNameHtml + 'calendar_' + cDate3 + '.html>' + a [4] + '</a><br>'
                    ...
                    if nDayOfWeek == 6:
                        ...
                        if nDay != nLastDay:
                            ...
                            cHtml = cHtml + '<tr><td>' + str(datetime.date(int(nYear), int(nMonth), int(nDay)).strftime('%V')) + cWeekLines + '</td>'
                            #cHtml = cHtml + '<tr><td>' + datetime.datetime.date(nYear, nMonth, nDay).strftime('%V') + '</td>'

                            #cHtml = cHtml + '<tr><td></td>'

                    if len(cHtml2) > 0:
                        # date html
                        cHtml3 = '<html>' + cStyle + '<body>[cInstructorsMenu]<br><h3>' + str(nDay) + ' ' + lMonthNames[nMonth] + ' ' + str(nYear) + '</h3><br><table>'
                        cHtml3 = cHtml3 + '<tr><td><b>time</b><td><b>product</b><td><b>part</b><td><b>student</b><td><b>instructor</b></td><tr>'
                        cHtml3 = cHtml3 + cHtml2
                        cHtml3 = cHtml3 + '</table>'

                        cFileName = cFolderNameOS + '/calendar_' + cDate3 + '.html'

                        aHtmlFiles.append([cFileName, cHtml3])
    ...
    cInstructorsMenu = cInstructorsMenu + '<br>'

    # cHtml = cHtml.replace('[cInstructorsMenu]', cInstructorsMenu) 

    cFileName = cFolderNameOS + '/calendar_' + cDate4 + '_' + cTime + '.html'

    aHtmlFiles.append([cFileName, cHtml])
    ...
    for i in aInstructor:
        ...
        cFileName = cFolderNameOS + '/calendar_' + cInsName + '.html'

        cHtml = '<html>' + cStyle + '<body>[cInstructorsMenu]<br><h3>' + i [2] + '</h3><br><table>' + i [3] + '</table>'

        aHtmlFiles.append([cFileName, cHtml])

    for h in aHtmlFiles:

        cHtml = h [1].replace('[cInstructorsMenu]', cInstructorsMenu) 
        cHtml = cHtml.replace('</tr><tr><td colspan = 4>&nbsp;</td></tr></table>', '</tr></table>') 

        cHtml = cHtml + '<br><br><p>powered by divescheduling. created:' + cDate5 + ' ' + cTime2 + '</p><br>'
        cHtml = cHtml + '<br></body></html>'

        #print('saving cFileName: ', h [0], ' ', len(cHtml))
        f = open(h [0], 'w')
        f.write(cHtml)
        f.close()
    ...
    return render_template('test.html', lRBAC = lRBAC, cHtml = cHtml)

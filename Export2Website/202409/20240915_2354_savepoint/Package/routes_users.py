...
def usersedit2(id, cFrom):
    ...
    note = Notes.query.where(and_((Notes.User == user.Id),(Notes.Type == 'st'))).all() # .first()

    print('type(note): ', type(note))
    print('note: ', note)
    print('len(note): ' , len(note))
    print('note[0]: ', note[0]) # note[0]:  <Notes 1>

    print('note.Id: ', note.Id)       # AttributeError: 'list' object has no attribute 'Id'
    print('note[0][1]: ', note[0][1]) # TypeError: 'Notes' object is not subscriptable
    note = [n[0] for n in note]       # TypeError: 'Notes' object is not subscriptable
    print('note[1]: ', note[1])       # IndexError: list index out of range
    ...
    return render_template('userseditform2.html', form = form, user = user, listStatus = listStatus , lRBAC = lRBAC, note = cNote)

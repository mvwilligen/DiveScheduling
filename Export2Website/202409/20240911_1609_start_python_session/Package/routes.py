...
def appointmentsedit(id, cFrom):
    ...
    instructors2 = db.session.execute(db.select(Instructors.Id, Instructors.User, Instructors.Name).where(Instructors.Active).order_by(Instructors.Name))
    ...
    return render_template('appointmentsedit.html', form=form, appointment=appointment2, cDebug=cDebug, instructors=instructors, instructors2=instructors2, lRBAC = lRBAC)
...
def htmlexport():
            ...
            if True:
                ...
                for nDays in range(nLastDay):
                    ...
                    for a in appointments:
                        ...
                        # date html
                        cHtmlDat = cHtmlDat + "<tr><td>" + a [6].strftime('%d-%m-%Y %H:%M') + "</td><td>" + a[4] + "</td><td>" + a[5] + "</td><td>"  + a[2] + " " + a[3] + "</td><td>" + a[8] + "</td><td>" + a[11] + "</td>" + cCRLF
                        ...
                        if len(a[11]) > 0:
                            ...
                            if not a[11] in cInstructorsMenu + cAssistantsMenu:
                                print ('a[11]:  ', a[11])
                                cInstructorsMenu = cInstructorsMenu + ";<a href=" + cFolderNameHtml + "calendar_in_" + a[11].replace(' ', '_') + ".html>" + a[11] + "</a>"
                                nInstructor = nInstructor + 1
    ...
    cHeader = cHeader + "<a href='http://127.0.0.1:5000/' target='_blank'><img src='logo.png' alt='ds'></a></td><td style='border:none;'>" + cCRLF
    ...
    src = 'C:/Data/Python/DiveScheduling/Exports/calendar.png'

    dst = cFolderNameOSHtml + 'calendar.png'
    # print('src: ', src)
    # print('dst: ', dst)
    shutil.copyfile(src, dst)

    dst = cFolderNameOSInternal + 'calendar.png'
    # print('src: ', src)
    # print('dst: ', dst)
    shutil.copyfile(src, dst)
